...
class CrosswordModelTests: XCTestCase {
    ...
    func testSquaresShouldClues() throws {
        ...
        for num in gridNums {
            if num != 0 {
                XCTAssertTrue((Crossword.squareShouldHaveAcrossClue(squareNumber: i, cols: cols, grid: grid) || Crossword.squareShouldHaveDownClue(squareNumber: i, cols: cols, grid: grid)))
            }
            i += 1
        }
    }
    
    func testBuildNamesToCluesMap() throws {
        // This is an example of a functional test case.
        // Use XCTAssert and related functions to verify your tests produce the correct results.
        // Heuristic test
        var i = 0
